...
class DFlashVisualExplainer(VoiceoverScene):
    ...
    def setup(self):
        ...
        if provider == "openrouter":
            ...
            self.set_speech_service(
                OpenRouterSpeechService(
                    model=os.getenv("OPENROUTER_TTS_MODEL", DEFAULT_OPENROUTER_TTS_MODEL),
                    voice=os.getenv("OPENROUTER_TTS_VOICE", DEFAULT_OPENROUTER_TTS_VOICE),
                    speed=float(os.getenv("OPENROUTER_TTS_SPEED", "0.96")),
                    style=os.getenv("OPENROUTER_TTS_STYLE"),
                    style_degree=float(style_degree) if style_degree is not None else None,
                )
            )
            ...
            self._external_dir = Path("media/voiceovers/dflash_external").resolve()
